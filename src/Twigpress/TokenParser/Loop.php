...
class LoopTokenParser extends Twig_TokenParser
{
    ...
    public function parse(Twig_Token $token)
    {
        ...
        $body = $this->parser->subparse(array($this, 'decideLoopFork'));
        if ($this->parser->getStream()->next()->getValue() == 'else') {
            $this->parser->getStream()->expect(Twig_Token::BLOCK_END_TYPE);
            $else = $this->parser->subparse(array($this, 'decideLoopEnd'), true);
        } else {
            $else = null;
        }
        $this->parser->getStream()->expect(Twig_Token::BLOCK_END_TYPE);
    ...
    public function decideLoopFork(Twig_Token $token)
    {
        return $token->test(array('else', 'endloop'));
    }

    public function decideLoopEnd(Twig_Token $token)
    {
